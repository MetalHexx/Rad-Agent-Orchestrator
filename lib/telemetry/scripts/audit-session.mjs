...
// ---------------- pricing (USD per 1,000,000 tokens) — Anthropic docs, 2026-06 ----------------
// Ratios vs base input are identical across families (output 5x, write-5m 1.25x,
// write-1h 2x, read 0.1x); only the base rate differs. Source: platform.claude.com/docs pricing.
const PER_M = {
...
function dollarsFor(u, model) {
  const p = PER_M[familyOf(model)];
...
// input*1 + output*5 + cacheRead*0.1 + cacheCreation*1.25   (creation = write5m+write1h here)
function effectiveOf(u) {
  return u.input * 1 + u.output * 5 + u.read * 0.1 + (u.write5m + u.write1h) * 1.25;
}
...
function readRawAssistant(file) {
  ...
  for (const ln of text.split('\n')) {
    ...
    rows.push({ requestId: o.requestId, model: o.message?.model || 'unknown', isSidechain: !!o.isSidechain, u });
  }
  return rows;
}
...
function ourUsage(rec) {
  ...
  u.read = rec.cacheReadTokens || 0; u.write5m = rec.cacheCreationTokens || 0; // store flattens creation
  return u;
}
...
const truthDollars = [...truth.values()].reduce((s, v) => s + dollarsFor(v.u, v.model), 0);

// ---- per-model (truth) ----
const byModel = new Map();
for (const v of truth.values()) {
  const fam = familyOf(v.model);
  const e = byModel.get(fam) ?? { fam, count: 0, u: emptyU(), dollars: 0, sampleModel: v.model };
  e.count++; addU(e.u, v.u); e.dollars += dollarsFor(v.u, v.model);
  byModel.set(fam, e);
}
...
for (const m of mismatched.slice(0, 8)) {
  ...
  console.log(`       our : in=${m.b.input} out=${m.b.output} read=${m.b.read} create=${m.b.write5m}`);
}
console.log('');
...
for (const e of byModel.values()) {
  const base = PER_M[e.fam].input;
  const eff = effectiveOf(e.u);
  const implied = eff * base / 1e6; // exact == $ when no 1h-cache writes are present
  const flag = Math.abs(implied - e.dollars) < 1e-9 ? 'exact' : `Δ ${usd(e.dollars - implied)} (1h-cache)`;
