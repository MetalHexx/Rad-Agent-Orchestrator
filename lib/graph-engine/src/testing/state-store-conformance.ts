...
import type { DagNode } from '../model/node.js';
import { ROOT_NODE_ID, ROOT_TRAITS } from '../model/root.js';
import type { ProjectScope, StateStore } from '../store/state-store.js';
import type * as vitest from 'vitest';

/**
 * The slice of vitest's own types the suite needs, resolved via a type-only import of the
 * `vitest` module namespace so every matcher the ported cases use (`toEqual`, `toBe`, `toBeNull`,
 * `toHaveLength`, `toContain`, `toThrow`, `.not`) is typed correctly with nothing enumerated by
 * hand — and, being type-only, it erases at build, so this package takes on no vitest *runtime*
 * dependency.
 */
type VitestApi = Pick<typeof vitest, 'describe' | 'it' | 'expect'>;
...
/**
 * The store-agnostic behavioral suite every `StateStore` implementation must satisfy: seeding,
 * apply round-trip, scope isolation, transactional rollback, and edge/node referential integrity.
 * Asserts only the public `StateStore` surface (`getNode`/`listNodes`/`listEdges`/`apply`) plus
 * `ROOT_NODE_ID`/`ROOT_TRAITS` — never a store's internals — so the same cases run unmodified
 * against any conforming implementation. `api` is the vitest test API injected by the caller so
 * this package never imports `vitest` at runtime.
 */
export function runStateStoreConformance(makeStore: () => StateStore, api: VitestApi): void {
  const { describe, it, expect } = api;

  describe('StateStore conformance — seeding', () => {
  ...
  describe('StateStore conformance — apply round-trip', () => {
  ...
  describe('StateStore conformance — scope isolation', () => {
  ...
  describe('StateStore conformance — transactional rollback', () => {
  ...
  describe('StateStore conformance — edge/node referential integrity', () => {
