...
frozen wire-contract fixtures, the built-in event-token normalization, and all eight `rad-orc:*`
built-ins — `phase`, `master_plan`, `explosion`, `approval`, `task`, `code_review`, `corrective`,
