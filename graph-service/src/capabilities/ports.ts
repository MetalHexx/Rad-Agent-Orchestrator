...
import type { CapabilityPortSet } from '@rad-orchestration/graph-engine';

/** This package's own name for the engine's `CapabilityPortSet` — equal to it (never a re-declared shape) so it satisfies `ResolveContext.ports` structurally with no adapter. */
export type CapabilityPorts = CapabilityPortSet;
