...
// to completion (running the real explosion/audit/approval cycle) is P04-T02's own functional
// scenario; this only proves the fixture stamps what it claims to.
...
describe('fixture: plan subgraph seed', () => {
  ...
  it('seeds the spine in order, gated in a chain, with the frontier starting at master_plan', async () => {
    ...
    expect([masterPlan?.order, explosion?.order, planAudit?.order, planApproval?.order]).toEqual([0, 1, 2, 3]);
    ...
    expect(edgeKeys.has(`depends_on:${PLAN_SUBGRAPH_IDS.explosion}->${PLAN_SUBGRAPH_IDS.planAudit}`)).toBe(true);
    expect(edgeKeys.has(`depends_on:${PLAN_SUBGRAPH_IDS.planAudit}->${PLAN_SUBGRAPH_IDS.planApproval}`)).toBe(true);

    // The seeded frontier is master_plan alone — explosion/plan_audit/plan_approval are all still
    // gated behind their own unmet depends_on predecessor.
