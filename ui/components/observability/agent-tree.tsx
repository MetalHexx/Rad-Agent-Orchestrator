...
export interface AgentTreeProps {
  ...
  sessionId?: string;
  /** Called with the transcript id when the user clicks a row to inspect it (FR-3, AD-7). */
...
// Pure & reusable: no data fetch, no page/live imports; one flat AgentRow per node (AD-1).
export function AgentTree({ tree, title = 'Agent Breakdown', coverage, ready = true, now, sessionId, onInspect }: AgentTreeProps) {
  ...
  /** Build an inspect prop for a given transcript id (or undefined if not inspectable). */
  ...
  return (
    <section className={CARD}>
      ...
      <div className="px-3 pb-3 pt-2">
        ...
        {orderedSubagents.map((node) => {
          const transcriptId = sessionId ? rowTranscriptId(node, 'subagent', sessionId) : null;
          return (
            <AgentRow
              key={node.key}
              node={node}
              ...
              variant="subagent"
              now={now}
              inspect={inspectProp(transcriptId)}
            />
          );
        })}
      </div>
