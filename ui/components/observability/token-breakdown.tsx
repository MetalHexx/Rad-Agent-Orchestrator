...
export function TokenBreakdown({ input, output, cacheRead, cacheCreate, spend }: TokenBreakdownProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-1 text-xs text-muted-foreground tabular-nums">
      {CELLS.map((cell, i) => (
      ...
      <span>Folds into Total Spend {humanizeTokens(spend)} via cache-aware weighting</span>
    </div>
  );
}
