...
test('renders three cards in order: Token Spend, Subagents, Duration (FR-1, DD-1)', () => {
  const html = renderToStaticMarkup(createElement(SessionSummaryCards, { session }));
  const ts = html.indexOf('Token Spend'), su = html.indexOf('Subagents'), du = html.indexOf('Duration');
  assert.ok(ts !== -1 && su !== -1 && du !== -1, 'all three labels present');
  assert.ok(ts < su && su < du, 'ordered Token Spend → Subagents → Duration');
});
