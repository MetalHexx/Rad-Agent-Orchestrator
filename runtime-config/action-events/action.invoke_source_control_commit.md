...
If the agent returns an **off-branch escalation** for any repo (a detached-HEAD or wrong-branch report) in place of a normal commit row, treat it as a halt-worthy escalation: stop, surface the repo(s) by name, and do **not** record it as a commit or signal `commit_completed` for that repo. (FR-4, DD-2)
