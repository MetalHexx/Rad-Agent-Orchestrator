# Handoff-Original E2E — Runner Prompt

> **Arm A of an A/B pair.** This drives `@coder` against the **original** P02-T01 handoff (contracts
> named but not inlined). Arm B (`../handoff-improved-e2e/`) drives the *same* task with the
> **improved** handoff (contracts inlined). The sandbox and task are identical across both arms — the
> **only** variable is `fixtures/handoff.md`. Compare token/read spend across the two with your own
> benchmarking tool.
...
4. **Isolation fence (include verbatim in the spawn prompt):**
   ...
   > `output/run-*` folder, the sibling `handoff-improved-e2e/` behavior, or anything outside your
