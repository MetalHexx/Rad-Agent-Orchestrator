# masterplan-original-e2e — Runner Prompt

> **Arm A of an A/B pair.** One **general-purpose planning agent** authors a Master Plan from a
> fixed Requirements fixture using the **original** `rad-create-plans` master-plan guide (the
> pre-change baseline — no compile-complete **External surface**). Arm B
> (`../masterplan-improved-e2e/`) authors the *same* plan with the **improved** guide. Requirements,
...
5. **Isolation fence (include verbatim):**
   ...
   > any `output/run-*` folder, the sibling `masterplan-improved-e2e/` behavior, or any prior run,
