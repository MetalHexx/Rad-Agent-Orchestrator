...
| Project name | `RAD-MASTER-BENCH-V1` | Passed as `$0` to `/rad-plan`. Held constant so artifact filenames are identical and diffable. |
...
from `path.basename(--project-dir)` = `RAD-MASTER-BENCH-V1`, so doc filenames are stable.
...
1. **Doc-path escape guard.** The engine rejects any doc whose path resolves *outside*
   ...
   `output/run-N/RAD-MASTER-BENCH-V1/`. The Setup step guarantees this.
