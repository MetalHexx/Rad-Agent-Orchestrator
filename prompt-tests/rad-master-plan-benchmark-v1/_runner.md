# rad-master-plan-benchmark-v1 — Runner Prompt
...
This is the **V1** test: the fixture is in the **old FR/NFR/AD/DD ledger** format — the
shape the current master-plan skill was built around. (Its sibling **V2** seeds the new
`R{n}` format.) See `README.md` for the old-vs-new comparison.
...
| Input | Value | Notes |
|-------|-------|-------|
| Fixture | `rainbow-hello` | `fixtures/rainbow-hello/REQUIREMENTS.md` — old FR/NFR/AD/DD ledger (22 requirements). |
| Project name | `RAD-MASTER-BENCH-V1` | The engine derives it from `path.basename(--project-dir)`. Held constant so artifact filenames are identical and diffable. |
...
```
prompt-tests/rad-master-plan-benchmark-v1/output/run-N/RAD-MASTER-BENCH-V1/
```

The engine derives the project name from `path.basename(--project-dir)` = `RAD-MASTER-BENCH-V1`,
...
> `RAD-MASTER-BENCH-V1-REQUIREMENTS.md` resolves correctly. The V1 fixture ships
> `requirement_count: 22`.

## Setup (bootstrap)

1. Choose the run number `N` (lowest free `run-N` under
   `prompt-tests/rad-master-plan-benchmark-v1/output/`).
...
3. Stage the Requirements fixture into the project dir, renamed to the project convention:
   ```
   prompt-tests/rad-master-plan-benchmark-v1/fixtures/rainbow-hello/REQUIREMENTS.md
     → output/run-N/RAD-MASTER-BENCH-V1/RAD-MASTER-BENCH-V1-REQUIREMENTS.md
   ```
4. **Verify the skip will be accepted:** confirm the staged doc's frontmatter has an
   **unquoted, positive-integer `requirement_count`** (the fixture ships `22`). If it's
...
node ~/.claude/skills/rad-orchestration/scripts/radorch.mjs pipeline signal \
  ...
  --project-dir <DIR> \
  --doc-path RAD-MASTER-BENCH-V1-REQUIREMENTS.md
```
...
`RAD-MASTER-BENCH-V1-MASTER-PLAN.md`. Confirm it exists, then signal `master_plan_completed`
...
node ~/.claude/skills/rad-orchestration/scripts/radorch.mjs pipeline signal \
  ...
  --project-dir <DIR> \
  --doc-path RAD-MASTER-BENCH-V1-MASTER-PLAN.md
```
...
node ~/.claude/skills/rad-orchestration/scripts/radorch.mjs plan explode \
  --project-dir <DIR> \
  --master-plan <DIR>/RAD-MASTER-BENCH-V1-MASTER-PLAN.md \
  --project-name RAD-MASTER-BENCH-V1
```
...
> can start it with `/rad-ui-start` (check with `/rad-ui-status`). It serves on a
> configurable port (default 1337, via `ui.port` in `orchestration.yml`).

## Exit

Report to the operator:

- The run folder: `prompt-tests/rad-master-plan-benchmark-v1/output/run-N/RAD-MASTER-BENCH-V1/`
