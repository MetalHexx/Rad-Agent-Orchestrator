...
function makeFixtureRoot() {
  ...
  fs.writeFileSync(join(installerDir, 'manifests/v1.0.0-alpha.11.json'),
    JSON.stringify({ version: '1.0.0-alpha.11', channel: 'copilot-vscode-plugin', files: [] }));
  return root;
}
...
test('runBuild produces a full output/ tree with plugin.json under .claude-plugin/, .agent.md filenames, bundles, hooks, manifests (FR-26, DD-5)', async () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(join(out, 'manifests/v1.0.0-alpha.11.json')));
  } finally {
...
test('synthesized output/package.json declares the published name @rad-orchestration/copilot-vscode-plugin (FR-32, FR-33, DD-10)', async () => {
  ...
  try {
    ...
    assert.strictEqual(pkg.version, '1.0.0-alpha.11', 'version stamped from plugin.json');
