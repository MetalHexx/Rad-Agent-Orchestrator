...
test('emitManifest produces { version, channel, files[] } with bundlePath and destinationPath, omits user-data assets', async () => {
  ...
  try {
    ...
    await emitManifest({ harnessOutputDir: harnessOut, harness: 'claude', version: '1.0.0-alpha.11', manifestDir });
    const written = JSON.parse(fs.readFileSync(path.join(manifestDir, 'v1.0.0-alpha.11.json'), 'utf8'));
    assert.strictEqual(written.version, '1.0.0-alpha.11');
