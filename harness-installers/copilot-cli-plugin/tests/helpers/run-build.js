...
function stageFixture(root) {
  ...
  fs.writeFileSync(path.join(installerSrc, 'plugin.json'),
    JSON.stringify({ name: 'rad-orc', version: '1.0.0-alpha.11', author: { name: 'metalhexx' }, license: 'MIT' }));
  fs.mkdirSync(path.join(installerSrc, 'manifests'), { recursive: true });
  fs.writeFileSync(path.join(installerSrc, 'manifests/v1.0.0-alpha.11.json'),
    JSON.stringify({ version: '1.0.0-alpha.11', channel: 'copilot-cli-plugin', files: [] }));
  fs.writeFileSync(path.join(installerSrc, 'package.json'),
    JSON.stringify({
      name: '@rad-orchestration/copilot-cli-plugin-source',
      version: '1.0.0-alpha.11', private: true, type: 'module', license: 'MIT',
      engines: { node: '>=20' },
    }));
}
